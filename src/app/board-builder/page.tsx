...
export default function BoardBuilderPage() {
  ...
  const [boardData, setBoardData] = useState<{ strips: (string | null)[][]; order: RowOrder[] }>(
    () => ({
  ...
  const [strip3Enabled, setStrip3Enabled] = useState(true);
  const [history, setHistory] = useState<{ strips: (string | null)[][]; order: RowOrder[] }[]>([]);
  const [future, setFuture] = useState<{ strips: (string | null)[][]; order: RowOrder[] }[]>([]);
  ...
  const setBoardDataWithHistory = (
    updater:
      | { strips: (string | null)[][]; order: RowOrder[] }
      | ((prev: { strips: (string | null)[][]; order: RowOrder[] }) => { strips: (string | null)[][]; order: RowOrder[] })
  ) => {
    setBoardData((prev) => {
  ...
  const handleSelectSize = (s: Size) => {
    ...
    // Update the order array to reflect the alternating pattern of strips 1 and 2
  ...
  const handleChangeRowOrder = (rowIndex: number, stripNo: number) => {
    setBoardDataWithHistory((prev) => {
      const next = clone(prev);
      if (rowIndex >= 0 && rowIndex < next.order.length) {
        next.order[rowIndex] = stripNo;
      }
      console.log("Row order changed.", { rowIndex, stripNo, next });
      return next;
    });
  };
  ...
  return (
    <main className="relative grid grid-rows-[1fr_1fr] h-[100svh] w-full overflow-hidden">
      <BoardPreview isDrawerOpen={isOpen} onToggleDrawer={() => setIsOpen(v => !v)} boardData={boardData} size={size} onReorder={handleReorder} onReverseRow={handleReverseRow} />
      <StripBuilder boardData={boardData} setBoardData={setBoardDataWithHistory} strip3Enabled={strip3Enabled} onToggleStrip3={() => setStrip3Enabled((v) => !v)} />
      <Drawer isOpen={isOpen} onClose={() => setIsOpen(false)} onUndo={handleUndo} onRedo={handleRedo} canUndo={canUndo} canRedo={canRedo} onRandomize={handleRandomize} size={size} onSelectSize={handleSelectSize} />
    </main>
  );
}
